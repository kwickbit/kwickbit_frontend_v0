...
import { CircleMinus } from "@/components/common/AppIcon";
...
export const MyCollectionLine = ({
  ...
  const [accountingType, setAccountingType] = useState<AccountingTransactionType | null>(
    accountingLine.accountingType === undefined ? null : accountingLine.accountingType
  );
  ...
  const getAmount = (): number => {
    ...
    if (accountingLine.accountingType === undefined) {
      return 0;
    } else if (incomingTypes.includes(accountingLine.accountingType)) {
      return accountingLine.amountIncoming ?? 0;
    } else {
      return accountingLine.amountOutgoing ?? 0;
    }
  };
  ...
  ): ((account: AvailableAccount) => boolean) => {
    return (account) =>
      currencyMapping !== undefined &&
      account.accountType === 'Bank' &&
      account.currencyRef.reference ===
        currencyMapping?.integrationRef?.bank?.currencyRef.reference
  };

  const useSetResources = (): void => {
    ...
    switch (accountingType) {
      ...
      case AccountingTransactionType.Swap: {
        const currencyMapping = currencyMappings.find(
          currencyMapping =>
            keyFormatTransaction(currencyMapping.token) === keyFormatTransaction(token)
        );
        setResourcesOptions(accounts.filter(filterBankAccounts(currencyMapping)));
        break;
      }
      case null:
        break;
  ...
  useEffect(useSetResources, [colIdx, accountingType, accounts, bills, invoices, items, currencyMappings, transaction.tokenIncoming, transaction.tokenOutgoing, token]);

  const handleChangeAccountingType = (selectedAccountingType: AccountingTransactionType | null): void => {
    const updatedTransaction = {
      ...accountingLine,
      ...(
        selectedAccountingType ? { accountingType: selectedAccountingType } : {}
      )
    };
  ...
  const handleChangeResourceSelected = (selectedResource: Invoice | Bill | AvailableAccount | Item | null): void => {
    ...
    const updatedTransaction = {
      ...
      ...(
        selectedResource ?
          { resource: { name: resourceName, reference: resourceRef } } :
          {}
      )
    };
    updateLine(colIdx, updatedTransaction);
